...
class BST:
    ...
    def _contains_aux(self, node, key):
        if node is None:
            ...
            if key == node._value:
                ...
                if key < node._value:
                    result = self._contains_aux(node._left, key)
                else:
                    result = self._contains_aux(node._right, key)
    ...
    def _eq_aux(self, node1, node2):
        if node1 is None and node2 is None:
            ...
            ret = (node1._value == node2._value and
    ...
    def parent(self, key):
        ...
        while node is not None and value is None:
            if node._value > key:
                ...
                value = deepcopy(prev._value) if prev else None
                node = None
    ...
    def parent_r(self, key):
        ...
        # your code here
        parent, _ = self._parent_aux(None, self._root, key)
        return deepcopy(parent._value) if parent else None

    def _parent_aux(self, parent, node, key):
        if node is None:
            ...
            parent = node
            parent, node = self._parent_aux(node, node._left, key)
        elif node._value < key:
            parent = node
            parent, node = self._parent_aux(node, node._right, key)
    ...
    def _is_balanced_aux(self, node):
        if node is None:
            ...
            ret = (abs(self._node_height(node._left) - self._node_height(node._right)) <= 1 and
                   self._is_balanced_aux(node._left) and
    ...
    def _is_valid_aux(self, node, mini=float('-inf'), maxi=float('inf')):
        if node is None:
            ret = True
        else:
            if mini < node._value < maxi:
                ret = (self._is_valid_aux(node._left, mini, node._value) and
                       self._is_valid_aux(node._right, node._value, maxi))
            else:
                ret = False
        return ret
    ...
    def inorder(self):
        ...
        # your code here
        contents = []
        self._inorder_aux(self._root, contents)
        return contents

    def _inorder_aux(self, node, contents):
        if node is not None:
            self._inorder_aux(node._left, contents)
            contents.append(deepcopy(node._value))
            self._inorder_aux(node._right, contents)
    ...
    def preorder(self):
        ...
        """
        contents = []
        self._preorder_aux(self._root, contents)
        return contents

    def _preorder_aux(self, node, contents):
        if node is not None:
            contents.append(deepcopy(node._value))
            self._preorder_aux(node._left, contents)
            self._preorder_aux(node._right, contents)
    ...
    def postorder(self):
        ...
        """
        contents = []
        self._postorder_aux(self._root, contents)
        return contents

    def _postorder_aux(self, node, contents):
        if node is not None:
            self._postorder_aux(node._left, contents)
            self._postorder_aux(node._right, contents)
            contents.append(deepcopy(node._value))
