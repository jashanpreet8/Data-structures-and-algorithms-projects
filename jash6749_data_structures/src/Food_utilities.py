...
def get_food():
    ...
    veg = input("Vegetarian (Y/N): ").lower() == 'Y'

    cal = input("Calories: ")

    food = Food(name, int(origin), veg, int(cal))
...
def read_food(line):
    ...
    # Your code here
    name, origin, veg, cal = line.split('|')

    veg_value = veg.lower() == 'true'

    food = Food(name, int(origin), veg_value, int(cal))
...
def food_table(foods):
    ...
    # Your code here
    sort = sorted(foods, key=lambda food: food.name)

    print(f"{'Food':<35} {'Origin':<12} Vegetarian Calories")
    print(f"{'-'*35} {'-'*12} {'-'*10} {'-'*8}")
    for food in sort:
        print(
            f"{food.name:<35} {food.origin:<12} {str(food.is_vegetarian):>10} {food.calories:>8}")
    ...
    for food in foods:
        if food.origin == origin or origin == -1:
            if food.calories <= max_cals or max_cals == 0:
                if food.is_vegetarian == is_veg or is_veg == False:
                    result.append(food)
