...
class BST:
    ...
    def _are_flipped_aux(self, self_node, t_node):
        """
        -------------------------------------------------------
        ...
        if self_node is None and t_node is None:
            flipped = True
        elif self_node is None or t_node is None:
            flipped = False
        elif (self_node._value != t_node._value or
              not self._are_flipped_aux(self_node._left, t_node._right) or
                not self._are_flipped_aux(self_node._right, t_node._left)):
            flipped = False
    ...
    def _total_height_aux(self, node):
        ...
        if node is None:
            ...
            total = node._height
            total += self._total_height_aux(node._left)
            total += self._total_height_aux(node._right)
    ...
    def furthest(self):
        ...
        if self._root is not None:
            node_l = self._root
            while node_l._left is not None:
                node_l = node_l._left
            values.append(node_l._value)

            node_r = self._root
            while node_r._right is not None:
                node_r = node_r._right
            values.append(node_r._value)
